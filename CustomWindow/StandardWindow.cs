...
namespace CustomWindow
{
    public class StandardWindow : Window
    {
        ...
        #region MaximizeOverTaskbar Property
        /// <summary>
        /// MaximizeOverTaskbar
        /// <remarks>Optional setting to make the window not go over taskbar when maximized</remarks>
        /// </summary>
        public new Boolean MaximizeOverTaskbar
        {
            get { return (Boolean)GetValue(MaximizeOverTaskbarProperty); }
            set { SetValue(MaximizeOverTaskbarProperty, value); }
        }

        public static readonly DependencyProperty MaximizeOverTaskbarProperty = DependencyProperty.Register(
            "MaximizeOverTaskbar", typeof(Boolean), typeof(StandardWindow), new PropertyMetadata(true, MaximizeOverTaskbarChangedCallback));

        // called when 'MaximizeOverTaskbarChangedCallback' property changed
        static void MaximizeOverTaskbarChangedCallback(DependencyObject property, DependencyPropertyChangedEventArgs args)
        {
            StandardWindow window = (StandardWindow)property;
            if ((Boolean)args.NewValue)
            {
                window.SourceInitialized -= new EventHandler(window.OnSourceInitialized);
            }
            else
            {
                window.SourceInitialized += new EventHandler(window.OnSourceInitialized);
        ...
        // used when maximized window is set to not go over taskbar
        void OnSourceInitialized(object sender, EventArgs e)
        {
            System.IntPtr handle = (new WinInterop.WindowInteropHelper(this)).Handle;
            WinInterop.HwndSource.FromHwnd(handle).AddHook(new WinInterop.HwndSourceHook(CustomWindow.WinApi.WindowProc));
        }
